...

{
  ...
  if(nUnique > 1) { // The case when not all elements are equal.
    ...
    std::vector< std::vector< ldouble > > S( 1, std::vector<ldouble>(N) );
    std::vector< std::vector< size_t > > J( 1, std::vector<size_t>(N) );

    size_t Kopt;

    // Fill in dynamic programming matrix
    
    Kopt = EWL2::fill_dp_matrix(x_sorted, y_sorted, max_rmse, S, J, Kmax);

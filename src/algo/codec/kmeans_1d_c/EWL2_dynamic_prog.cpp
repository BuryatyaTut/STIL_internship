...
long long fill_dp_matrix(const std::vector<double> & x,
                         // data
                     ...
                     */
{
        ...
        long double max_se = max_rmse * max_rmse * N;
#ifdef DEBUG
        ...
        for(long long q = 1; q < K; ++q) {
				...
				if (max_se >= S[q-1][N-1]) //early exit for when we're ok with current rmse
				{
						S.clear(); //S is not needed anymore
						return q;
				}
    ...
    S.clear(); //S id not needed anymore
    return K;
